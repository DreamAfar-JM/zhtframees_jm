...
<script type="text/javascript">
	function doSubmit() {
		if (!ZHTAJAX.validateFromCallback($("#formss"),
				ZHT.ajaxDoneAndCloseDialog)) {
...
<div class="easyui-layout" data-options="fit:true,border:false">
	<div data-options="region:'center',border:false ," title="" style="overflow: hidden;padding: 10px;">
		<form id="formss" method="post" action="${ctx}/common/demo/duser/addDuser">
			<input type="hidden" name="id" value="${duser.id}" />
			<fieldset>
				<legend>
					<img src="resources/images/fromedit.png" style="margin-bottom: -3px;" />测试用例1添加
				</legend>
				<table>
					<tr>
						<th>用户名:</th>
						<td><input type="text" name="name" value="${duser.name}" class="easyui-textbox validatebox"
					...
					<tr>
						<th>联系方式:</th>
						<td><input type="text" name="mobile" value="${duser.mobile}" class="easyui-textbox validatebox"
							data-options="  validType:'mobile'" /></td>

						<th>住址:</th>
						<td><input type="text" name="address" value="${duser.address}" class="easyui-textbox validatebox"
