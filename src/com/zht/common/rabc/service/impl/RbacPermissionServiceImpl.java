...
public class RbacPermissionServiceImpl extends BaseServiceImpl<RbacPermission> implements IRbacPermissionService {
		...
		for (Class<?> calzz : calssSet) {
			...
			for (Method method : methods) {
				...
				if (method.isAnnotationPresent(RequiresPermissions.class)) {
					...
					if (methodRPerms != null && methodRPerms.length > 1) {// 多个
																			// pemis
	...
	public List<String> findAllPermsUserHaveAndInDefaultRoleInPatternA(String userName) {
		...
		// String
		// hqlFromRole=" select p.code from RbacPermission p join p.rbacRoles r where r.id = (select u.defaultRbacRole.id from RbacUser u where u.userName =:userName)";
		List<String> listFromDefaultRole = (List<String>) rbacPermissionDao.findJustList(hqlFromDefaultRole,
	...
	public List<String> findAllPermsUserHaveAndInDefaultRoleInPatternB(String userName) {
		...
		// String
		// sql=" SELECT  per.code from rbac_permission per,rbac_role_permission rper "
		...
		// + " ) ";
		String hqlFromDefaultPosition = "" + " select rp.rbacPermission.code from RbacRolePermission rp " + " where rp.rbacRole.id in ( "
	...
	public DataSet findPermissionDataSetInUserPermissionByUserId(ParamObject po, Long userId) {
		...
		String hql = "select " + "up.rbacPermission.id as id, " + "up.rbacPermission.code as code, " + "up.rbacPermission.name as name, "
				+ "up.rbacPermission.enabled as enabled, " + "up.rbacPermission.url as url, " + "up.rbacPermission.type as type, "
				+ "up.rbacPermission.description as description " + "@from RbacUserPermission up where up.rbacUser.id=:userId";
		if (!ZBeanUtil.isEmptyValue(po.getReqParam("enabled"))) {
			hql += " and up.rbacPermission.enabled=:enabled ";
			po.addAllowNull("enabled", po.getReqParam("enabled"));
		}
	...
	public DataSet findPermissionDataSetInUserPermissionRejectByUserId(ParamObject po, Long userId) {
		...
		String hql = "select " + "upr.rbacPermission.id as id, " + "upr.rbacPermission.code as code, " + "upr.rbacPermission.name as name, "
				+ "upr.rbacPermission.enabled as enabled, " + "upr.rbacPermission.url as url, " + "upr.rbacPermission.type as type, "
				+ "upr.rbacPermission.description as description " + "@from RbacUserPermissionReject upr where upr.rbacUser.id=:userId";
		if (!ZBeanUtil.isEmptyValue(po.getReqParam("enabled"))) {
			hql += " and upr.rbacPermission.enabled=:enabled ";
			po.addAllowNull("enabled", po.getReqParam("enabled"));
		}
	...
	public DataSet findPermissionDataSetNotInUserPermAndUserPermReject(ParamObject po, Long userId) {
		...
		po.initType(POType.H_O_C);
		String hql = "select " + "p.id as id, " + "p.code as code, " + "p.name as name, " + "p.enabled as enabled, " + "p.url as url, "
				+ "p.type as type, " + "p.description as description " + "@from RbacPermission p where 1=1";
